...
function insert_style(){
    var result = document.getElementById("result_tabnabbing_detection");
    if(result)
        return;
    ...
    result.style.visibility = "hidden"; // hidden by default
...
// detect if the current tab has focus or not
document.addEventListener('visibilitychange',()=>{
    insert_style();
    // if the current page does not have focus, hide the page
...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        if (request.greeting == "success"){
            resemble(request.arg1).compareTo(request.arg2).onComplete((data)=>{
                ...
                if(diff_percent > 20){
                    ...
                    chrome.runtime.sendMessage({greeting:"green"},(response)=>{
                        if(chrome.runtime.lastError)
                            return;
                    });
                }
